/* @ts-self-types="./jspi_fetch_streams.d.ts" */
 ...
 */
export async function fetch_stream(url) {
    const ptr0 = passStringToWasm0(url, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = await (__wbg_jspi_fetch_stream ??= WebAssembly.promising(wasm.fetch_stream))(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {Promise<Array<any>>}
 */
export async function read_stream(stream) {
    const ret = await (__wbg_jspi_read_stream ??= WebAssembly.promising(wasm.read_stream))(stream);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
function __wbg_get_imports() {
    const import0 = {
        ...
        __wbg_getReader_e782bd33388a2c7b: function(arg0) {
    ...
    return {
        __proto__: null,
        "./jspi_fetch_streams_bg.js": import0,
    };
}

let __wbg_jspi_fetch_stream;

let __wbg_jspi_read_stream;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('jspi_fetch_streams_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
