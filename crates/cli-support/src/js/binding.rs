...
use crate::wit::{AuxImport, InstructionData};
...
fn instruction(
    ...
    match instr {
        ...
        | Instruction::DeferFree { .. } => {
            ...
            let call = invoc.invoke(id, js.cx, &args, &mut js.prelude, log_error)?;
    ...
    fn invoke(
        ...
        match self {
            Invocation::Core { id, .. } => {
                ...
                let code = if cx.jspi_call_exports.contains(adapter_id) {
                    format!("WebAssembly.promising(wasm.{})({})", name, args.join(", "))
                } else {
                    format!("wasm.{}({})", name, args.join(", "))
                };
                Ok(code)
            }
            Invocation::Adapter(id) => {
                ...
                if let AuxImport::Intrinsic(Intrinsic::JspiCall) = import {
                    cx.jspi_call_instr = Some(*adapter_id);
                }
