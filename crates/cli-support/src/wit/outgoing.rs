...
impl InstructionBuilder<'_, '_> {
    ...
    fn outgoing_ref(&mut self, mutable: bool, arg: &Descriptor) -> Result<(), Error> {
        match arg {
            ...
            // (one of the concrete `impl IntoWasmAbi for &$t` impls generated by
            // `ref_into_wasm_abi_for_scalar!`), so the wire is identical to
            // passing `T` by value — JS just receives the primitive. Only the
            // shared-ref form participates; `&mut primitive` has no such ABI.
            //
            // The accepted set lives in `is_scalar_by_shared_ref` below and must
            // match the `ref_into_wasm_abi_for_scalar!` list in
            ...
            // `T` that has no by-reference wire representation. The fixed list in
            // `ref_into_wasm_abi_for_scalar!` is meant to keep that set in
            // lockstep with the arms above, so this means either that list grew a
            // type that isn't really scalar, or the two lists have drifted. Say
            // what *is* supported rather than only dumping the internal
            // descriptor.
            _ => bail!(
...
/// `ref_into_wasm_abi_for_scalar!` list in `src/convert/impls.rs`, which
/// decides for which `T` an `impl IntoWasmAbi for &T` exists at all. If this
/// function accepts less than that list, a program compiles and then fails in
/// the CLI; if it accepts more, this arm is dead. The `scalar-ref-args`
/// reference test binds every type in that list, so it fails if this function
/// is narrower; `scalar_by_shared_ref_set_is_exactly_the_scalars` below pins
/// this side.
///
...
fn is_scalar_by_shared_ref(d: &Descriptor) -> bool {
    match d {
        ...
        // applies to `Clamped<T>`; `ref_into_wasm_abi_for_scalar!` has no entry
        // for `Clamped<u8>`, so there is no `impl IntoWasmAbi for &Clamped<u8>`
        // and `Ref(ClampedU8)` is unreachable. Excluded on purpose.
        Descriptor::ClampedU8
