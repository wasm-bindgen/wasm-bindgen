        ...
        for (idx, (sig_comment, key, signature, orig_func_ids)) in sorted.into_iter().enumerate() {
            ...
            match key {
                ...
                GenericImportKey::Shim(shim) => {
                    ...
                    // `assert_no_shim` is rejected in combination with
                    // `generic_per_mono` by the macro (see `parser.rs`), so it is
                    // always false on this path.
                    self.finish_import_binding(id, aux_import, catch, variadic, false);
                }
            }
