...
impl Bindgen {
    ...
    pub fn generate_output(&mut self) -> Result<Output, Error> {
        ...
        // Error if reinit hooks were defined without --experimental-reset-state-function.
                ...
                if aux.reinit_preinit.is_some() || aux.reinit_postinit.is_some() {
                    bail!(
                        "pre_reinit_hook / post_reinit_hook are defined but \
                         --experimental-reset-state-function was not passed; \
                         the hooks will never be called"
                    );
                }
            }
        }
