...
extern "C" {
    ...
    #[wasm_bindgen(method, generic_per_mono, js_name = raw)]
    fn raw_pointer<T>(this: &Shaped<*const T>);

    #[wasm_bindgen(method, generic_per_mono, js_name = function)]
    fn function_pointer<T>(this: &Shaped<fn(T)>);

    #[wasm_bindgen(method, generic_per_mono, js_name = dynamic)]
