...
/// Derive macro for `Upcast` trait implementations including `Upcast<Object>`.
///
/// This is the standard derive for JavaScript types. It provides full covariance including
/// upcasting to Object and JsValue.
///
/// # Example
///
/// ```rust,ignore
...
/// pub struct MyJsType { /* ... */ }
///
/// let my_type: MyJsType = /* ... */;
/// let obj: Object = my_type.clone().upcast(); // ✓ Works!
/// let js_val: JsValue = my_type.upcast(); // ✓ Works!
/// ```
#[proc_macro_derive(Upcast)]
pub fn derive_upcast(item: TokenStream) -> TokenStream {
    match wasm_bindgen_macro_support::expand_upcast(item.into()) {
        Ok(tokens) => {
            if cfg!(xxx_debug_only_print_generated_code) {
                println!("{tokens}");
            }
...
/// Derive macro for `Upcast` trait implementations (core version without Object covariance).
///
/// Use this when js-sys is not available as a dependency. For most cases, use `#[derive(Upcast)]` instead.
///
/// This provides JsValue covariance, identity covariance, and structural covariance, but does not
/// include covariance to Object.
///
/// # Example
///
/// ```rust,ignore
...
/// let js_val: JsValue = my_type.upcast(); // ✓ Works!
///
/// let container: MyContainer<Number> = /* ... */;
/// let wider: MyContainer<JsValue> = container.upcast(); // ✓ Works!
/// ```
#[proc_macro_derive(UpcastCore)]
pub fn derive_upcast_core(item: TokenStream) -> TokenStream {
    match wasm_bindgen_macro_support::expand_upcast_core(item.into()) {
        Ok(tokens) => {
            if cfg!(xxx_debug_only_print_generated_code) {
                println!("{tokens}");
            }
