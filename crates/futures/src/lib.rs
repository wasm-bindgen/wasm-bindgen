...
use wasm_bindgen::{prelude::*, GenericCast, JsUpcastRef, JsValueCast};
...
impl<T> From<Promise<T>> for JsFuture<T>
where
    T: GenericCast + FromWasmAbi + 'static,
...
where
    ...
    T: Into<JsValue> + JsValueCast + JsUpcastRef,
{
    let mut future = Some(future);
