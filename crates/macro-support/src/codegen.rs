...
impl TryToTokens for ast::ImportType {
    fn try_to_tokens(&self, tokens: &mut TokenStream) -> Result<(), Diagnostic> {
        ...
        if !class_generic_params.is_empty() {
            ...
            let concrete_defaults: Vec<_> = class_generic_params.iter().map(|(_, d)| d).collect();
            let ty_generics_concrete = quote! { <#(#concrete_defaults),*> };
            ...
            generic_or_concrete_impls = quote! {
                ...
                #[automatically_derived]
                impl From<JsValue> for #rust_name #ty_generics_concrete {
                    #[inline]
