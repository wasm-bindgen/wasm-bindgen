...
struct Cli {
    ...
    bench: bool,
    #[arg(long, requires = "bench", help = "Run benchmarks with codspeed")]
    codspeed: bool,
...
fn rmain(cli: Cli) -> anyhow::Result<()> {
    ...
    let benchmark = std::env::var("WASM_BINDGEN_BENCH_RESULT_ROOT")
        .map_or_else(|_| env::current_dir().unwrap(), PathBuf::from)
