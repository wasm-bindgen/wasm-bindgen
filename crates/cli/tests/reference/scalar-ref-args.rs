...
// `ref_into_wasm_abi_for_scalar!` let a shared reference to a scalar be passed
// to JS by copying the value, so the wire form is identical to passing `T` by
// value.
//
// Every type in the `ref_into_wasm_abi_for_scalar!` list in
// `src/convert/impls.rs` appears below, on purpose. That list and
// `is_scalar_by_shared_ref` in `crates/cli-support/src/wit/outgoing.rs` have to
// stay in lockstep: if the Rust side admits a `T` the CLI does not, the program
// compiles and then fails at wasm-bindgen time with "unsupported type behind a
// reference". Binding all sixteen here turns that class of drift into a test
// failure. Adding a type to `ref_into_wasm_abi_for_scalar!` means adding it here
// too.
//
