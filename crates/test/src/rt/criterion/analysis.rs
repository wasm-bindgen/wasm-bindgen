use crate::__rt::criterion::{__wbg_collect_raw_walltime_results, __wbg_set_executed_benchmark};
...
use super::{compare, prev, Criterion, SavedSample};
...
pub(crate) fn common<M: Measurement>(
    ...
    if let Some(location) = &criterion.location {
        let uri = format!("{}::{}::{}", location.file, location.module, id.desc());
        __wbg_set_executed_benchmark(uri.clone());
        let iters_per_round = iters.iter().map(|t| t.to_string()).collect();
        let times_per_round_ns = times.iter().map(|t| t.to_string()).collect();
        __wbg_collect_raw_walltime_results(
            "criterion".into(),
            id.desc().into(),
            uri,
            iters_per_round,
