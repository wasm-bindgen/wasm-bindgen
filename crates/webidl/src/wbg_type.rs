        ...
        match self {
            ...
            WbgType::Callback {
                ...
                if generics_compat {
                    ...
                    // New mode: with generics
                    // Generate js_sys::Function<Return, A1, A2, ..., A9>
                    // Use Callback position for both arguments and returns (conservative generalization)

                    // Convert return type (defaults to Never if undefined)
                    let ret_ty = match return_type {
                        Some(rt) => {
                            // Use Callback position: primitives → JS types (Number, Boolean, JsString)
                            rt.to_syn_type(TypePosition::Callback, legacy, false)?
                                .unwrap_or_else(|| parse_quote!(::wasm_bindgen::JsValue))
                        }
                        None => {
                            // undefined → Never (void returns, unused return slot)
                            parse_quote!(::js_sys::Never)
                        }
                    };
                    ...
                    // Pad with Never to reach 9 params (indicates unused parameter slots)
                    while param_types.len() < 9 {
                        param_types.push(parse_quote!(::js_sys::Never));
                    }

                    // Build js_sys::Function<Return, A1, A2, ..., A9>
