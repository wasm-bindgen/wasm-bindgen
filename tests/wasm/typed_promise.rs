...
async fn test_typed_promise_basic() {
    let promise = Promise::resolve(&42i32);
    checkPromise(&promise).await.unwrap();

    let out = JsFuture::from(promise.clone()).await.unwrap();
    assert_eq!(out, 42i32);

    let closure = Closure::new(|num: i32| (num + 1) as f64);
    let result_promise = promise.then(&closure);
    let out = JsFuture::from(result_promise).await.unwrap();
    assert_eq!(out, 43f64);

    let promise = Promise::resolve(&JsValue::from(42i32));
    let typed_promise: Promise<i32> = promise.dyn_into().unwrap();
    checkPromise(&typed_promise).await.unwrap();

    let string_promise = Promise::resolve(&JsString::from("test"));
...
async fn test_typed_promise_option() {
    let promise = Promise::resolve(&Some(JsString::from("test")));
    ...
    let promise: Promise<Option<i32>> = Promise::resolve(&Some(4));
    let closure = Closure::new(|opt_num: Option<i32>| {
        assert_eq!(opt_num.unwrap(), 4);
    });
    let result_future: JsFuture<_> = promise.then(&closure).into();
    result_future.await.unwrap();
}
...
async fn test_typed_promise_different_types() {
    ...
    let closure = Closure::new(|num| num * 2.0);
    let result_promise = typed_promise.then_map(&closure);
...
async fn test_typed_promise_js_objects() {
    ...
    let arr = Array::new_t::<i32>();
    arr.push(&1);
    arr.push(&2);
    arr.push(&3);

    let promise: Promise<Array<i32>> = Promise::resolve(&arr);

    let closure = Closure::new(|arr: Array<i32>| arr.length());
    let result_promise = promise.then(&closure);
...
async fn test_typed_promise_round_trip() {
    ...
    let result = JsFuture::from(final_typed).await.unwrap();
...
async fn test_typed_future_to_promise_chaining() {
    ...
    let chained = typed_promise.then_map(&closure);
